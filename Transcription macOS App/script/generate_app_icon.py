...
def stamp_circle(pixels: bytearray, size: int, cx: float, cy: float, radius: float) -> None:
    pad = 1.25
    min_x = int(math.floor(cx - radius - pad))
    max_x = int(math.ceil(cx + radius + pad))
    min_y = int(math.floor(cy - radius - pad))
    max_y = int(math.ceil(cy + radius + pad))
...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: generate_app_icon.py <iconset-dir>", file=sys.stderr)
        return 2

    iconset = Path(sys.argv[1])
    iconset.mkdir(parents=True, exist_ok=True)
    for name, size in ICON_FILES.items():
        write_png(iconset / name, size, size, render(size))
    return 0
