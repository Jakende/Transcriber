...
media_paths = filedialog.askopenfilenames(
    title="Select one or more audio or video files for transcription",
    filetypes=[
        (
...
if not media_paths:
    print("No file(s) selected. Aborting.")
    raise SystemExit(0)
...
INCLUDE_TIMECODES = messagebox.askyesno(
    "Timecodes",
    "Write timecodes to the Markdown transcript?",
    parent=root,
)
...
for index, media_path in enumerate(media_paths, start=1):
    ...
    print(f"\nTranscription {index}/{len(media_paths)}: {media_name}")
...
"""
    ...
    if index < len(media_paths):
        print(f"Waiting {BULK_FILE_BUFFER_SECONDS} seconds before the next file ...")
